...
export default class Plaid extends Component {
  ...
  sendPublicToken = () => {
    ...
    axios
      .post("http://169.253.111.137:4000/sendToken", {
        token: this.state.data.metadata
      })
      .then(response => {
        console.log("HELLO HIIIIIIIII", response.data);
        axios
          .get("http://169.253.111.137:4000/getUserDonations")
          .then(response =>
  ...
  renderLogin() {
    return (
      <WebView
        source={{
          uri: `https://cdn.plaid.com/link/v2/stable/link.html?key=${PLAID_PUBLIC_KEY}&env=${PLAID_ENV}&product=${PLAID_PRODUCT}&clientName=Give&isWebView=true&isMobile=true&webhook=http://google.com`
        }}
